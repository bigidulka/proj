...
class ViewTestPage(QWidget):
    ...
    def initUI(self):
        ...
        label = QLabel(f'Посмотреть тест: {self.test_name}', self)
...
class CreateTestPage(QWidget):
    ...
    def validate_test(self):
        if not self.test_name.text():
            QMessageBox.warning(self, 'Ошибка проверки', 'Укажите название теста.')
            return False

        if self.attempts_spinbox.value() <= 0:
            QMessageBox.warning(self, 'Ошибка проверки', 'Количество попыток должно быть больше 0.')
            return False

        if len(self.answer_widgets) < 1:
            QMessageBox.warning(self, 'Ошибка проверки', 'Требуется хотя бы один вопрос.')
            return False

        for group_box, answer_widgets in self.answer_widgets.items():
            question_text = group_box.findChild(QTextEdit).toPlainText()
            if not question_text:
                QMessageBox.warning(self, 'Ошибка проверки', 'Каждый вопрос должен иметь заголовок.')
                return False
            if not answer_widgets['answers']:
                QMessageBox.warning(self, 'Ошибка проверки', 'Каждый вопрос должен иметь хотя бы один вариант ответа.')
                return False
            
            if len(answer_widgets['answers']) < 2:
                QMessageBox.warning(self, 'Ошибка проверки', 'Каждый вопрос должен иметь не менее двух вариантов ответа.')
                return False
            
            if not any(info['correct'].isChecked() for info in answer_widgets['answers']):
                QMessageBox.warning(self, 'Ошибка проверки', 'В каждом вопросе должен быть отмечен хотя бы один правильный ответ.')
                return False
    ...
    def addQuestion(self):
        ...
        question_text_edit.setPlaceholderText("Введите сюда текст вашего вопроса...")
        group_box_layout.addWidget(question_text_edit)

        question_type = QComboBox()
        question_type.addItems(["Единственный правильный ответ", "Несколько правильных ответов"])
        ...
        self.answer_widgets[group_box] = {'layout': answers_layout, 'type': 'Единственный правильный ответ', 'answers': []}
